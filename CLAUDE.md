...
1. **`data/data-<num>-<view>.json`** — the **only** source of textual content. The `<num>` prefix is the section number from the project outline (e.g. `data-1.1.1-pestel.json`, `data-2.1.1-mercado-objetivo-caracteristicas.json`) and determines the order in which the JSON is concatenated into the unified `public/erp-evolutivo.md` doc (the build script does `readdirSync().sort()`). Plain text strings, structured by what the view needs to show. **Never** HTML tags, **never** Tailwind classes, **never** color tokens, **never** `testId`s. Tags like `FASE 1`, `CANAL #1`, `FIJO`, `PRINCIPAL` are content (they appear on screen) and stay in the JSON; the color used to render them is a presentation decision and lives in the component.
2. **`src/app/features/<view>/<view>.component.{ts,html}`** — imports the JSON (`import data from '../../../../data/data-<num>-<view>.json'`), declares the typed interfaces for the JSON shape, maps semantic identifiers (card id, tag value, factor id) to design-token classes/colors, and renders the template by iterating over the data. The folder and component name stay as `<view>` without the numeric prefix.

When adding a new view: pick the section number from the outline in `.claude/skills/nueva-vista/SKILL.md` (table "Outline del proyecto"), create the JSON file first with the full content, then write the component that consumes it. Refactoring existing views means moving copy out of the template into the JSON without changing what renders. See `data/data-0-canvas.json` + `features/canvas/canvas.component.ts` for the reference pattern, and `data/data-1.1.1-pestel.json` + `features/analisis-estrategico/pestel/pestel.component.ts` for a view with typed block sequences (`paragraph` / `list` / `note` / `info` / `implication`).
